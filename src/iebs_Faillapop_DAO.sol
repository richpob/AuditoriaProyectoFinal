...
contract FP_DAO {
    ...
    modifier isAuthorized(string calldata magicWord) {
        require(keccak256(abi.encodePacked(magicWord)) == keccak256(abi.encodePacked(password)), "Unauthorized");
        _;
    }
        ...
        returns (uint256)
    {
        ...
        disputes[dId] = Dispute(itemId, buyerReasoning, sellerReasoning, 0, 0, 0);
    ...
    function lotteryNFT(address user) internal {
        uint256 randomNumber =
            uint8(uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), block.timestamp, user))));
